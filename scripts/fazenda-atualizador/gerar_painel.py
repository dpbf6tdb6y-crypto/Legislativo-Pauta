...
<style>
  ...
  *{ box-sizing:border-box; margin:0; padding:0; }
  ...
  .marca{ font-size:15px; font-weight:600; letter-spacing:-.01em; white-space:nowrap; }
  .marca em{ font-style:normal; color:var(--t3); font-weight:400; margin-left:9px;
             font-size:12px; }
...
function colunasMes(host,dados,sel,onClick){
  host.innerHTML='';
  ...
  dados.forEach((d,i)=>{
    ...
    const marcado = sel && sel===d.mes;
    if(h>0){
      const r=Math.min(6,bw/2,h);
      const p=S('path',{d:'M'+(cx-bw/2)+','+base+' L'+(cx-bw/2)+','+(y+r)
        +' Q'+(cx-bw/2)+','+y+' '+(cx-bw/2+r)+','+y
        +' L'+(cx+bw/2-r)+','+y+' Q'+(cx+bw/2)+','+y+' '+(cx+bw/2)+','+(y+r)
        +' L'+(cx+bw/2)+','+base+' Z', class:'cb'+((!sel||marcado)?' on':'')});
      ...
      dica(p, d.k, '<em>'+exato(d.v)+'</em><br>'+(soma?f1.format(d.v/soma*100)+'% do período':'')
           +(onClick?'<br><span style="opacity:.6">clique para ver só este mês</span>':''));
      svg.appendChild(p);
